...
class _ParentStaffState extends State<ParentStaff> {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
      future: FirebaseFirestore.instance.collection("DaycareAddStaff").where("DaycareId",isEqualTo: daycreID).get(),
      builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return Center(
              child: CircularProgressIndicator(
        ...
        return Scaffold(
          appBar: AppBar(
            backgroundColor: Color.fromRGBO(238, 54, 74, 1),
            toolbarHeight: 150,
            title: Row(
              children: [
                SizedBox(width: MediaQuery.of(context).size.width * .04),
                            ...
                            leading: Container(
                              ...
                              decoration: BoxDecoration(
                                  ...
                                  image: DecorationImage(
                                      fit: BoxFit.fill,
                                      image:
                                          NetworkImage(pstaff[index]["Path"]))),
                            ),
                            // Image.network(pstaff[index]["path"]),
                            title: Text(pstaff[index]["StaffName"],
